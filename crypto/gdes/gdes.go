...
func EncryptCBC(plainText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	...
	if len(iv) != block.BlockSize() {
		return nil, fmt.Errorf("iv length invalid")
	}
...
func EncryptCBCTriple(plainText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
		return nil, fmt.Errorf("key length invalid")
	}
...
func Padding(text []byte, padding int) ([]byte, error) {
	...
	case NOPADDING:
		if len(text)%8 != 0 {
			return nil, fmt.Errorf("text length invalid")
		}
	case PKCS5PADDING:
		return PaddingPKCS5(text, 8), nil
	default:
		return nil, fmt.Errorf("padding type error")
	}
